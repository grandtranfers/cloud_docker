...
function searchAll(keyword){
	...
	document.write("<tr><td><center><h3>Resultados <b>"+(display_start+1)+"-"+(displast)+"</b> de <b>"+sites+"</b> para <b>"+keyword+"</b> Tiempo de busqueda <b>"+((timeB-timeA)/1000)+"</b> segundos.</h3></center></td></tr>")

	if (displast>sites && finds!=0) displast=sites+1


	if (finds==0) {
		document.write("<tr><td><table width=50%' align='center'><tr><td><h3><font color=green>no se encontró  <b>''"+keyword+"''</b></font></h3>"+	"<p>La palabra - <b>"+keyword+"</b> - no se encuentra en la base de datos.</p>"+
	...
	for (q=display_start; q<q3; q+=1) {
		if (results[q].val>0) {
			rc=results[q].rec
			document.write("<tr> <td><span class='xtitle'>"+records[rc].link+"</span><br>")
			...
			if (x1>0 && x2>0) {
				...
				document.write("<table border=0 width=500><tr><td><span class='xresult'>"+records[rc].description+"</span></td></tr></table><span class='xlocation'>"+tmp+"</span><br><br></td></tr>")
			}
			q2++
   		}
	}
	...
	if (finds>10) {
		...
		if (display_start>0) document.write("<a href='"+paramstring+"&disp="+(display_start-10)+"'>anterior hola 1</a>")
		document.write("&nbsp;&nbsp;&nbsp;")
		...
		document.write("&nbsp;&nbsp;&nbsp;")
		if (displast<=sites) document.write("<a href='"+paramstring+"&disp="+(displast)+"'>siguiente hola2</a>")
...
function AddBody() {
	...
	templateBody()
	document.write("<tr height='80px'><td><div class='container'><form name='searchform' method='post' action='javascript:doSearch()'><input  name='searchwords' type='text' placeholder='Buscar' size='30'>&nbsp;&nbsp;<a href='javascript:doSearch()'>"+ButtonCode+"</a></form></div></td></tr><tr><td><hr size=1> </td></tr>")
...
function DisplayXSearch() {
	...
	document.write("<tr><td><h3><center>Haga clic aqui  para bajar <a href=xsearch.zip>XSearch "+version+"</a> Creado por P. Bestebroer. Modificado por L. Aybar</center></h3></td></tr>")
	templateEnd()
}
